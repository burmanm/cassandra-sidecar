...
/**
 * Handler for management-api compatible garbage collect endpoint.
 */
public class ManagementGarbageCollectTablesHandler extends AbstractHandler<ManagementGarbageCollectTablesHandler.RequestParams>
implements AccessProtected
{
    @Inject
    public ManagementGarbageCollectTablesHandler(InstanceMetadataFetcher metadataFetcher,
    ...
    {
        String tombstoneOption = context.request().getParam("tombstoneOption");
        if (tombstoneOption == null || tombstoneOption.trim().isEmpty())
        {
            tombstoneOption = "ROW";
        }
        if (!"ROW".equalsIgnoreCase(tombstoneOption) && !"CELL".equalsIgnoreCase(tombstoneOption))
        {
            throw new IllegalArgumentException("tombstoneOption must be either ROW or CELL");
        }

        try
        {
            ManagementKeyspaceRequest request = Json.decodeValue(context.body().asString(), ManagementKeyspaceRequest.class);
            return new RequestParams(request, tombstoneOption.toUpperCase());
        }
    ...
    static class RequestParams
    {
        ...
        final String tombstoneOption;

        RequestParams(ManagementKeyspaceRequest keyspaceRequest, String tombstoneOption)
        {
